...
import com.adv.core.normalmap.NormalMappingRenderer
import com.adv.core.shaders.StaticShader
import com.adv.core.shaders.TerrainShader
import com.adv.core.shadows.ShadowMapMasterRenderer
...
class MasterRenderer(loader: Loader, camera: Camera?) {
    ...
    private val multiMeshEntities: MutableMap<MultiModelKey, MutableList<MultiMeshEntity>> =
        HashMap()
    ...
    fun renderScene(
        ...
        normalEntities: MutableList<Entity>,
        multiMeshEntities: MutableList<MultiMeshEntity>,
    ...
    fun processNormalMapEntity(entity: Entity) {
        for (model in entity.GetModels()) {
            val batch = normalMapEntities[model]
            if (batch != null) {
                batch.add(entity)
            } else {
                val newBatch: MutableList<Entity> = ArrayList()
                newBatch.add(entity)
                normalMapEntities[model] = newBatch
            }
        }
    }
